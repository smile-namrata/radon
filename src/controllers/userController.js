...
const loginUser = async function (req, res) {
  try {
    ...
    let user = await userModel.findOne({
      ...
      if (!user)
      return res.status(400,"bad request").send({ status: false, msg: "username or the password is not corerct", });
  ...
  // ---------[Create Token JWT]---------
    ...
    res.status(500).send({ msg: "error", problem: err.message });
  }
};
...
const deleteUser = async function (req, res) {
  ...
  let userId = req.params.userId;
    ...
    res.status(500).send({ msg: "server errorr", error: err.message });
  }
};
